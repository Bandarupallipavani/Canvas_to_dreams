...
import { ShoppingBag, User, Menu, X, Palette, LogOut, LayoutDashboard, Home } from 'lucide-react'
  ...
  return (
    <>
      <nav className="bg-white/95 backdrop-blur-sm border-b border-canvas-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/" className="flex items-center gap-2 group flex-shrink-0">
              ...
              <div>
                <div className="font-display font-bold text-ink text-base sm:text-lg leading-none">Canvas to Dreams</div>
                <div className="text-xs text-ink-muted font-body leading-none hidden sm:block mt-1">Original Paintings</div>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Mobile menu (Keep only for toggleable info, though bottom nav handles pages) */}
              <button
        ...
        {/* Mobile Nav */}
        {open && (
          <div className="md:hidden border-t border-canvas-100 py-3 space-y-1 px-4 sm:px-6">
            {navLinks.map(l => (
              <Link
                key={l.to}
                to={l.to}
      ...
      <div className="fixed bottom-4 left-4 right-4 z-50 bg-white/90 backdrop-blur-md border border-canvas-100 shadow-[0_8px_30px_rgba(26,18,8,0.12)] rounded-2xl md:hidden px-4 py-2.5 flex items-center justify-around">
        <Link
          to="/"
          className={`flex flex-col items-center gap-1 transition-colors ${
            pathname === '/' ? 'text-canvas-600' : 'text-ink-muted hover:text-ink'
          }`}
        >
        ...
        <Link
          to="/shop"
          className={`flex flex-col items-center gap-1 transition-colors ${
            pathname.startsWith('/shop') ? 'text-canvas-600' : 'text-ink-muted hover:text-ink'
          }`}
        >
        ...
        <Link
          ...
          className={`relative flex flex-col items-center gap-1 transition-colors ${
            pathname === '/cart' ? 'text-canvas-600' : 'text-ink-muted hover:text-ink'
          }`}
        >
          <ShoppingBag className="w-5.5 h-5.5" />
          {count > 0 && (
            <span className="absolute -top-1 right-2 w-4 h-4 bg-canvas-600 text-parchment text-[9px] font-bold rounded-full flex items-center justify-center animate-pulse">
              {count > 9 ? '9+' : count}
            </span>
          )}
        ...
        <Link
          ...
          className={`flex flex-col items-center gap-1 transition-colors ${
            pathname === '/profile' || pathname === '/login' ? 'text-canvas-600' : 'text-ink-muted hover:text-ink'
          }`}
        >
